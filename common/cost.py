...
def obj(layout):	# OBJECTIVE FUNCTION THAT WE'RE TRYING TO MINIMISE
	...
	L_coll = 0
	for i in range(N):
		L_coll += np.sqrt(layout[2*i]**2 + layout[2*i+1]) # Assuming turbine-wise coordinates
	...
	C_elec = L_coll*(0.0105*10000 + 95) + np.ceil(0.603*P_rated*N/100000 + 0.464)*((0.00168*100000 + 1380)*1000 + 0.668*100000 + 36000 + 1.035*(1000*(P_rated)**0.751)*N)
