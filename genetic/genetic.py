...
class Population:
	...
	def initialise_random(self):
		for i in range(self.n_pop):
			self.pop_matrix[i, 1:] = initial_random_layout.random_layout(self.N, self.x_bound, self.y_bound, self.grid_size).layout
	...
	def crossover(self):
		...
		for i in range(self.n_pop):
			...
			new_pop_matrix[i, 1:] = beta*self.pop_matrix[r[1], 1:] + (1-beta)*self.pop_matrix[r[2], 1:]
